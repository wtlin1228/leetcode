...
For example, the minimum absolute difference of the array `[5,2,3,7,2]`
is `|2 - 3| = 1`. Note that it is not `0` because `a[i]` and `a[j]` must
be different.
...
elements of `nums` between the **0-based** indices `li` and `ri`
(**inclusive**).
